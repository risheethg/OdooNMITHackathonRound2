...
class UserService:
    ...
    def create_user(self, data: CreateUserSchema):
        try:
            ...
            # Create user in local DB only
            result = self.repo.create(user_data)
            new_id = result.inserted_id

            logs.define_logger(level=20, loggName=inspect.stack()[0], message=f"User created with ID: {new_id}")

            return response.success(data={"id": str(new_id)}, message="User created successfully", status_code=201)
        
        except Exception as e:
            logs.define_logger(level=40, loggName=inspect.stack()[0], message=f"Error creating user: {e}", body=data.model_dump_json(exclude={'password'}, exclude_none=True))
    ...
    def get_user_by_id(self, item_id: str):
        ...
        try:
            ...
            if user_doc:
                # Use the response schema to ensure sensitive data is not returned
